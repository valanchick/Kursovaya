...
// константные значени€ дл€ ползунка скорости
...
//класс дл€ создани€ ползунка дл€ выбора скорости от 1.0 до 2.5 по определЄнным меткам
class Slider {
private:
...
public:
    ...
    //обновл€ем информацию о местоположении 
    void updateVisuals() {
...
void getInput(sf::RenderWindow& window, unsigned int screenWidth, unsigned int screenHeight, unsigned int size[], SimulationType& simType) {
    ...
    const float scrollSpeed = 10.0f; //смещение текста в пол€х ввода
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                if (event.mouseButton.button == sf::Mouse::Left) {
                    ...
                    // проверка нажати€ на кнопку "Ќазад"
...
//вычисление размера клетки в зависимости от размеров пол€
...
//класс дл€ правил цветной симул€ции
